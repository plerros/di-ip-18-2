...
//======================================================================================================================================================
int operatorchk(int current){if(current == '+' || current == '-' || current == '*'){return 1;} else return (0);}
int numberchk(int current)  {if(current >= '0' && current <= '9'){return 1;}		       else return (0);}
int signchk(int current)    {if(current == '+' || current == '-'){return 1;}		       else return (0);}
int spacechk(int current)   {if(current == ' ' || current == '\t'){return 1;}		       else return (0);}
int endchk(int current)     {if(current == '\n'|| current == ')'){return 1;}		       else return (0);}

//======================================================================================================================================================
int input(int *error){
	int in = getchar();
	if(in == EOF){*error = 5; return 0;}
	else if(!operatorchk(in) && !numberchk(in) && !spacechk(in) && !endchk(in) && in != '('){*error = 1;}
	return(in);
}
//======================================================================================================================================================
int handlerror(int *error, int *parenthesiscount){
	if(*error){																	//error 0 == no error
		if	(*error == 1){printf("error: Valid characters are: {+,-,*,1..9,(,),space,tab,newline}.\n");}					//error 1 == invalid input
		else if (*error == 2){printf("error: One operator between a pair of numbers, or between a number and  parenthesis, is needed.\n");}	//error 2 == missing operator
		else if (*error == 3){printf("error: A number or parenthesis open is expected after an operator or a sign.\n");}			//error 3 == missing number
		else if (*error == 4){printf("error: Empty input ' ' or '( )' isn't a valid mathematical expression.\n");}					//error 4 == empty calcline initialization
		else if (*error == 5){return 0;}													//error 5 == no error just EOF
		else if (*error == 6){printf("error: There can be at most one sign per number\n");}							//error 6 == too many signs
		return 0;
	}
	else{
		if (*parenthesiscount > 0){	printf("error: Missing ')'.\n");}									//error :more '(' than ')'
		else if (*parenthesiscount < 0){printf("error: Missing '('.\n");}									//error :less '(' than ')'
		return 0;
	}
}
//======================================================================================================================================================
int signadd(int current)	{if(signchk(current)){	return (current);} 		else return (0);}
int signn(int sign, int number) {if(sign){		return (number * (44 - sign));} else return (number);}
...
int get_operator(int *error){
	...
	if(*error) return (0);
		...
		*error = 2; return 0;
		...
		ungetc(current, stdin); return (0);
	}
	else if (operatorchk(current)){return(current);}
	return (0);
}
//======================================================================================================================================================
int get_number(int current, int *error, int *parenthesiscount){
	...
	if(current == '('){
		...
		if(sign){
			...
			if(*error) return (0);
			else if(current == '\n' || current == ')'){ ungetc(current, stdin); *error = 3; return (0); }
			else if(signchk(current)){*error = 6; return (0); }
		}
...
int calcline(int prevnumber, int operator, int *error, int *parenthesiscount){		
	int number, operatornew, current = spaceskip(input(error), error); if(*error) return (0);
#ifdef DEBUG
													int i;for(i=0;i<*parenthesiscount;i++){printf("\t");} printf("-  -  -  -  -  -  -  -  -  \n");for(i=0;i<*parenthesiscount;i++){printf("\t");} printf("initial prevnumber is: %d\noperator is: %c\n", prevnumber, operator);
#endif
	if(endchk(current)){ungetc(current, stdin); *error = 4; return 0;}
	...
	while(!endchk(current)){
		...
		else if (operator == '-'){prevnumber -= number;}
		else if (operator == '*'){if(operatornew){ungetc(operatornew, stdin);}return (prevnumber * number);}
